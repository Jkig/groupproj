...
function App() {
  ...
  const [speaker, setSpeaker] = useState(["You", "AI", "You", "AI", "You"])
  const [spanish, setSpanish] = useState(true)

  
  const indecies = [0,1,2,3,4]// have this match size of those things
  const incorrect = [false, false, false, true, false]
  ...
  return (
    <div className='body'>
      <ul className='vertical'>
        ...
        {indecies.map(index => <Response key={index} textNative={textNative[index]} textTarget={textTarget[index]} speaker={speaker[index]} incorrect={incorrect[index]}/>)}
        <ul>
          <li className="horizontal"><button onClick={() => setSpanish(!spanish)} className='langButton'>Speaking in {spanish ? "Spanish" : "English"}</button></li>
          <li className="horizontal"><button onClick={() => console.log("stoptalking")} className='stopButton'>stopSpeaking</button></li>
        </ul>
      </ul>
    </div>
  )
